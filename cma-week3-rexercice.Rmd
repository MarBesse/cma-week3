...
# posmo_filter <- posmo_filter %>%
#   mutate(
#     n_plus1 = sqrt((lead(X, 2)-X)^2 + (lead(Y, 2)-Y)^2), #from n to n+1
#     n_plus2 = sqrt((lead(X, 4)-X)^2 + (lead(Y, 4)-Y)^2), #from n to n+2
#     n_plus3 = sqrt((lead(X, 6)-X)^2 + (lead(Y, 6)-Y)^2), #from n to n+3
#     n_minus1 = sqrt((lag(X, 1)-X)^2 + (lag(Y, 1)-Y)^2), #from n to n-1
#     n_minus2 = sqrt((lag(X, 4)-X)^2 + (lag(Y, 4)-Y)^2), #from n to n-2
#     n_minus3 = sqrt((lag(X, 6)-X)^2 + (lag(Y, 6)-Y)^2) #from n to n-3
#   )
# 
# posmo_filter
# ```
...
posmo_filter_filter <- posmo_filter |>
    filter(!static)
...
```{r}
summary(posmo_filter_filter$stepMean)
```
